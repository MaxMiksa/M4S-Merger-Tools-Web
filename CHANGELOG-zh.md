...
## v2.0.0 – 性能与可靠性升级 (2025年12月6日)

### 增强特性：智能 M4S 混流
- **总结**: 实施了智能混流策略，在显著提升性能的同时，确保了 M4S 视频和音频流的兼容性和可靠性。
- **解决痛点**: 解决了不必要的音频重编码这一主要性能瓶颈，极大地加速了混流过程。提供了强大的回退机制以实现最大兼容性。
- **功能细节**:
  - **优先极速模式**: 工具现在首先尝试对视频和音频进行全流复制 (`-c copy`)。这利用了大多数 M4S 流预编码的特性，实现了近乎即时的混流。
  - **自动回退**: 如果快速流复制模式遇到兼容性问题（例如，不常见的音频编码），它会优雅地回退到更安全的重编码模式 (`-c:v copy -c:a aac -strict experimental`)，确保合并任务能够成功完成。
  - **改善用户体验**: 用户将受益于兼容文件显著更快的合并速度，并在无需手动干预的情况下无缝处理边缘情况。
- **技术实现**:
  - 修改了 `ffmpegService.ts::mergeFiles`，在 FFmpeg `run` 命令周围添加了 `try-catch` 块。
  - 第一次尝试使用 `['-c', 'copy']`。
  - 失败时，第二次尝试使用 `['-c:v', 'copy', '-c:a', 'aac', '-strict', 'experimental']`（如果视频和音频输入都存在）。
